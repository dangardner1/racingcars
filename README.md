...
- **Environmental hazards** — hills, tunnels, bridges, spike pits, crumbling bridges, boost pads
...
Tracks are data-driven JSON files interpreted by `src/game/Track.js`: a track is an ordered, closed loop of centerline nodes (`path`, each with `x/y/z/width/type`). Elevation differences between nodes are hills for free; `type` per node picks `flat`/`gap`/`bridge`/`tunnel`/`crumblingBridge`. Hazards (spike pits, boost pads) are positioned by node index. New hazards or tracks can be added without touching the renderer or physics code.
